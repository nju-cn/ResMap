...
from integral_executor import IntegralExecutor, IntegralJob, get_ipt_from_video, Job, Executor
...
def test_fixed_jobs():
    ...
    for fid in range(5):
        ...
        for wk, d_job in enumerate(wk2job):
            ...
            print(f"Frame{fid} Worker{wk} error: "
                  f"{[torch.mean(torch.abs(id2opt[oid] - results[oid])) for oid in d_job.out_ids]}")
            if wk + 1 < len(wk2job):
